    //
//  YDBulkAddictionViewController.m
//  YiDa
//
...
@interface YDBulkAddictionViewController()
...
- (void)bulkAddictionAddNewrecipe:(NSNotification *)_notice;
- (void)bulkAddictionRemoveNewRecipe:(NSMutableDictionary *)_info;
...
@implementation YDBulkAddictionViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        networkProcessQueue = dispatch_queue_create("YDBulkAddictionQueue.YDBulkAddictionViewController", NULL);
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(bulkAddictionAddNewrecipe:) name:YDBulkAddictionAddRecipeNotification object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(bulkAddictionRemoveNewRecipe:) name:YDBulkAddictionRemoveRecipeNotification object:nil];
...
//    [networkDataMemberHandle GetChemicalInfo:@"VL40289" Type:@"4"];
//    [self queryQueryChemicalWith:@"C2097378" type:@"4"];
    
//     [self queryQueryChemicalWith:scanedCode_ type:@"4"];
}
...
- (void)bulkAddictionAddNewrecipe:(NSNotification *)_notice
{
//    YDPalettesRecipeView *group = [[YDPalettesRecipeView alloc] initWithFrame:CGRectMake((176+16)*(recipeCount%4), (227+23)*(int)floor(recipeCount/4), 176, 227) recipeType:YDPalettesOldRecipeType];
//
//    [recipeScrollView_ addSubview:group];
//    [group release];
//    recipeCount += 1;
...
- (void)viewDidUnload {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:YDBulkAddictionAddRecipeNotification object:nil];
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:YDBulkAddictionRemoveRecipeNotification object:nil];
...
//        [self queryQueryChemicalWith:@"VL40289" type:@"0"];
        [self queryQueryChemicalWith:scanedCode_ type:@"4"];
    }
...
- (void)queryQueryChemicalWith:(NSString *)_colorCode type:(NSString *)_type result:(void (^)(ChemicalInfo *))block
{
    ...
    dispatch_async(networkProcessQueue,^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [loadingView removeFromSuperview];
            //            NSLog(@"获取调方信息信息   。。。。。%@",resutl);
//            NSLog(@"bulk获取加成配方信息   。。。。GroupInfoAry.count。 %d  ChemicalInfoAry.count .%d",resutl.GroupInfoAry.count,resutl.ChemicalInfoAry.count);
            block(resutl);
        });
...
- (IBAction)outputButtonPress:(id)_sender
{
    ...
    for (int i = 0; i<[newSelectRecipesList count]; i++)
    {
        ...
        dispatch_async(networkProcessQueue,^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [loadingView removeFromSuperview];
                __block YDBulkAddictionViewController *blockSelf = self;
